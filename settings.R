...
settings["eta"] <- sample(seq(0.0065, 0.0085, by=0.0005),1)
settings["min_child_weight"] <- 6 # sample(c(25,100,seq(6,10)),1) # 6
settings["max_depth"] <- sample(c(9,10),1)
...
settings["valpct"] <- sample(c(10.3283734188981, 10, 5), 1)
settings["corr_pct"] <- 50 # % used to test correlation
settings["subsample"] <- sample(c(0.7,0.8),1)
settings["colsample_bytree"] <- sample(seq(0.5,0.8,by=0.1),1)
settings["addGeoFields"] <- sample(c(T,F),1)
